...
namespace UserControls
{
    ...
    public partial class PostList : UserControl
    {
        ...
        private void BindPosts()
        {
            if (this.Posts == null)
            {
                var keyword = Request.Cookies["keyword"] != null ? Request.Cookies["keyword"].Value : string.Empty; ;
                var searchField = Request.Cookies["searchField"] != null ? Request.Cookies["searchField"].Value : string.Empty;
                int chapterId = 0;
                if (Request.Cookies["chapterId"] != null)
                    int.TryParse(Request.Cookies["chapterId"].Value, out chapterId);
                var verseFrom = 0;
                if (Request.Cookies["verseFrom"] != null)
                    int.TryParse(Request.Cookies["verseFrom"].Value, out verseFrom);
                var verseTo = 0;
                if (Request.Cookies["verseTo"] != null)
                    int.TryParse(Request.Cookies["verseTo"].Value, out verseTo);

                SearchByField searchType;
                Enum.TryParse(searchField, true, out searchType);

                Posts = FilterPosts(Post.ApplicablePosts, keyword, searchType, chapterId, verseFrom, verseTo)
                    .ConvertAll(p => p as IPublishable);
            }
