...
app.controller('PostEditorController', ["$rootScope", "$scope", "$location", "$filter", "$log", "dataService", function ($rootScope, $scope, $location, $filter, $log, dataService) {
    ...
    $scope.save = function () {
        ...
        var verses = $('#form').find('#txtVerses').val();
        var validationResult = versesValidCheck(verses);
       
        if (validationResult != 'Ok') {
            var errorMessage = '';
            if (validationResult == "Duplicated") {
                errorMessage = "Verses are duplicated.";
            } else {
                errorMessage = "Invalid \'Verses\' value. Exaple of valid value: 2, 6, 13-20, 24-28.";
            }
            $('#txtVerses').parent().append('<label id="txtVerses-error1" class="error" for="txtVerses"> ' + errorMessage + '</label>');
            $('#txtVerses').addClass('error');
            return false;
        }
    ...
    function versesValidCheck(value) {
        var listOfChapters = [];
        if (value != "") {
            //replace last comma
            ...
            if (splitedValue.length > 0) {
                for (var i = 0; i < splitedValue.length; i++) {
                    var trimmed = splitedValue[i].trim();
                    if (trimmed.indexOf('-') > 0) {

                        var diapason = trimmed.split('-');
                        if (diapason.length != 2 ||
                            !isIntegerCheck(diapason[0]) ||
                            !isIntegerCheck(diapason[1])) {
                            return "Invalid";
                        } else {
                            var fromDiapason = parseInt(diapason[0], 10);
                            var toDiapason = parseInt(diapason[1], 10);
                            if (fromDiapason > toDiapason) {
                                return "Invalid";
                            }
                            for (var verse = fromDiapason;  verse <= toDiapason; verse++) {
                                if (listOfChapters.indexOf(verse) >= 0)
                                    return "Duplicated";
                                else
                                    listOfChapters.push(verse);
                            }
                        }
                    } else if (isIntegerCheck(trimmed)) {
                        var intValue = parseInt(trimmed, 10);
                        if (listOfChapters.indexOf(intValue) >= 0)
                            return "Duplicated";
                        else
                            listOfChapters.push(intValue);
                    } else {
                        return "Invalid";
                    }
                }
    ...
    $(document).ready(function () {
        ...
        $('#form').validate({
            rules: {
                txtChapterNumber: { required: true },
                txtVerses: { required: true },
    ...
    app.directive('tbxVerses', function () {
        return {
            require: 'ngModel',
