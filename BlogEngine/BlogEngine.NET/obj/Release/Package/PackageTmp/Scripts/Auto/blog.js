...
BlogEngine = {
    ...
    // Searches the blog based on the entered text and
    // searches comments as well if chosen.
    customSearch: function (root, searchType, searchValue, chapterId, verseFrom, verseTo, onlySummary) {
        //var input = this.$(searchfield);
        //var check = this.$("searchcomments");
        //var searchField = this.$(searchFieldId);
        //var searchValue = $(searchField.options[searchField.selectedIndex]).attr('name');

        var searchPageExtension = typeof BlogEngineRes.fileExtension === "undefined" ? ".aspx" : BlogEngineRes.fileExtension;
        var keyword = (searchValue == undefined || searchValue == '') ? 'none' : searchValue;

        var search = "search" + searchPageExtension +
        ...
        if (verseFrom != undefined && verseFrom != '') {
            search += "&verseFrom=" + verseFrom;
        }
       
        if (verseTo != undefined && verseTo != '') {
            search += "&verseTo=" + verseTo;
        }
       
        search += "&onlySummary=" + onlySummary;

        //top.location.href = root + search;

        setCookieValue('keyword', encodeURIComponent(keyword));
        setCookieValue('searchField', searchType);
        setCookieValue('chapterId', chapterId);
        setCookieValue('verseFrom', verseFrom);
        setCookieValue('verseTo', verseTo);
        setCookieValue('onlySummary', onlySummary);
	...
	,
    // Clears the search fields on focus.
    customSearchClear: function (defaultText, searchfield) {
        if (!searchfield) {
            searchfield = 'searchfield';
        }
        var input = this.$(searchfield);
