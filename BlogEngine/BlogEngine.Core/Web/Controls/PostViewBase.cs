    ...
    public class PostViewBase : UserControl
    {
        ...
        public string Summary
        {
            get
            {
                ...
                if (Blog.CurrentInstance.IsSiteAggregation)
                {
                    summary = Utils.ConvertPublishablePathsToAbsolute(summary, post);
                }
                ...
                if (ShowExcerpt)
                {
                    ...
                    //else
                    //{
                    summary = Utils.StripHtml(summary);
                    //if (summary.Length > DescriptionCharacters && DescriptionCharacters > 0)
                    //{
                    //    summary = string.Format("{0}...{1}", summary.Substring(0, DescriptionCharacters), link);
                    //}
                    //}
                }
                return summary;
        ...
        protected override void OnLoad(EventArgs e)
        {
            ...
            var bodyContentShort = (PlaceHolder)FindControl("BodyContentShort");
            if (bodyContentShort != null)
                bodyContentShort.Controls.Add(new LiteralControl(FormatShortText(Body)));

            var bodySummary = (PlaceHolder)FindControl("BodySummary");
            if (bodySummary != null)
                bodySummary.Controls.Add(new LiteralControl(Summary));
        }

        private string FormatShortText(string content)
        {
            var text = Utils.StripHtml(content);
            if (text.Length > 200)
            {
