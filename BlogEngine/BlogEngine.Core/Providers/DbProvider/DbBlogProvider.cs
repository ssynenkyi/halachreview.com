    ...
    public partial class DbBlogProvider : BlogProvider
    {
        ...
        public override bool SetupNewBlog(Blog newBlog, string userName, string email, string password)
        {
            ...
            using (var conn = this.CreateConnection())
            {
                if (conn.HasConnection)
                {
                    ...
                    //todo slavik check summary
                    //post.Summary = newBlog.Summary;
        ...
        public override Post SelectPost(Guid id)
        {
            ...
            using (var conn = this.CreateConnection())
            {
                if (conn.HasConnection)
                {
                    //todo slavik add new Property here
                    var sqlQuery = string.Format("SELECT PostID, Title, Description, PostContent, DateCreated, DateModified, Author, IsPublished, IsCommentEnabled, Raters, Rating, Slug, IsDeleted, ChapterNumber, Verses, Summary FROM {0}Posts WHERE BlogID = {1}blogid AND PostID = {1}id", this.tablePrefix, this.parmPrefix);
                    using (var cmd = conn.CreateTextCommand(sqlQuery))
                    {
                        cmd.Parameters.Add(conn.CreateParameter(FormatParamName("blogid"), Blog.CurrentInstance.Id.ToString()));
                        ...
                        using (var rdr = cmd.ExecuteReader())
                        {
                            if (rdr.Read())
                            {
                                ...
                                if (!rdr.IsDBNull(4))
                                {
                                    post.DateCreated = BlogSettings.Instance.FromUtc(rdr.GetDateTime(4));
                                }
                                ...
                                if (!rdr.IsDBNull(13))
                                {
                                    post.ChapterNumber = rdr.GetInt32(13);
                                }
                                if (!rdr.IsDBNull(14))
                                {
                                    post.Verses = rdr.GetString(14);
                                }
                                if (!rdr.IsDBNull(15))
                                {
                                    post.Summary = rdr.GetString(15);
                                }
                            }
                        }
                        ...
                        //// Categories
                        //cmd.CommandText = string.Format("SELECT Number FROM {0}PostVerses WHERE BlogID = {1}blogid AND PostID = {1}id", this.tablePrefix, this.parmPrefix);
                        //using (var rdr = cmd.ExecuteReader())
                        //{
                        //    while (rdr.Read())
                        //    {
                        ...
                        //        if (key > 0)
                        //        {
                        //            post.VersesList.Add(new PostVerse
                        //            {
                        //                Number = key,
                        //                BlogID = Blog.CurrentInstance.Id.ToString(),
                        //                PostID = id.ToString()
                        //            });
                        //        }
                        //    }
                        ...
                        // Verses
                        cmd.CommandText = string.Format("SELECT Number FROM {0}PostVerses WHERE BlogID = {1}blogid AND PostID = {1}id AND IsDeleted = 0 ", this.tablePrefix, this.parmPrefix);
                        using (var rdr = cmd.ExecuteReader())
                        {
                            post.VersesList = new StateList<PostVerse>();
                            while (rdr.Read())
                            {
                                post.VersesList.Add(new PostVerse
                                {
                        ...
                        //        comment.DateCreated = BlogSettings.Instance.FromUtc(rdr.GetDateTime(1));
                        //        comment.Parent = post;
        ...
        public override void InsertPost(Post post)
        {
            using (TransactionScope ts = new TransactionScope())
            {
                using (var conn = this.CreateConnection())
                {
                    if (conn.HasConnection)
                    {
                        //todo slavik add new FieldAccessException( ) here
                        var sqlQuery = string.Format("INSERT INTO {0}Posts (BlogID, PostID, Title, Description, PostContent, DateCreated, DateModified, Author, IsPublished, IsCommentEnabled, Raters, Rating, Slug, IsDeleted, ChapterNumber, Verses, Summary) VALUES ({1}blogid, {1}id, {1}title, {1}desc, {1}content, {1}created, {1}modified, {1}author, {1}published, {1}commentEnabled, {1}raters, {1}rating, {1}slug, {1}isdeleted, {1}chapterNumber, {1}verses, {1}summary)", this.tablePrefix, this.parmPrefix);
                        ...
                        using (var cmd = conn.CreateTextCommand(sqlQuery))
                        {
                            ...
                            parms.Add(conn.CreateParameter(FormatParamName("summary"), post.Summary));
                            ...
                            parms.Add(conn.CreateParameter(FormatParamName("chapterNumber"), post.ChapterNumber));
                            parms.Add(conn.CreateParameter(FormatParamName("verses"), post.Verses));
                            cmd.ExecuteNonQuery();
                        }
        ...
        public override void UpdatePost(Post post)
        {
            using (TransactionScope ts = new TransactionScope())
            {
                using (var conn = this.CreateConnection())
                {
                    if (conn.HasConnection)
                    {
                        //todo slavik add new field here
                        var sqlQuery = string.Format("UPDATE {0}Posts SET Title = {1}title, Description = {1}desc, PostContent = {1}content, DateCreated = {1}created, DateModified = {1}modified, Author = {1}Author, IsPublished = {1}published, IsCommentEnabled = {1}commentEnabled, Raters = {1}raters, Rating = {1}rating, Slug = {1}slug, IsDeleted = {1}isdeleted, chapterNumber = {1}chapterNumber, verses = {1}verses, summary = {1}summary WHERE BlogID = {1}blogid AND PostID = {1}id", this.tablePrefix, this.parmPrefix);
                        ...
                        using (var cmd = conn.CreateTextCommand(sqlQuery))
                        {
                            ...
                            p.Add(conn.CreateParameter(FormatParamName("summary"), post.Summary));
                            ...
                            p.Add(conn.CreateParameter(FormatParamName("chapterNumber"), post.ChapterNumber));
                            p.Add(conn.CreateParameter(FormatParamName("verses"), post.Verses));
                            cmd.ExecuteNonQuery();
                        }
        ...
        /// The update categories.
        /// </summary>
        private void UpdateVerses(Post post, DbConnectionHelper conn)
        {
            var sqlQuery = string.Format("DELETE FROM {0}PostVerses WHERE BlogID = {1}blogid AND PostID = {1}id", this.tablePrefix, this.parmPrefix);
            using (var cmd = conn.CreateTextCommand(sqlQuery))
            {
                cmd.Parameters.Add(conn.CreateParameter(FormatParamName("blogid"), Blog.CurrentInstance.Id.ToString()));
                cmd.Parameters.Add(conn.CreateParameter(FormatParamName("id"), post.Id.ToString()));
                ...
                foreach (var verse in post.VersesList)
                {
                    cmd.CommandText = string.Format("INSERT INTO {0}PostVerses (BlogID, PostID, Number, DateCreated, IsDeleted) VALUES ({1}blogid, {1}id, {1}number, {1}datecreated, 0)", this.tablePrefix, this.parmPrefix);
                    cmd.Parameters.Clear();
                    cmd.Parameters.Add(conn.CreateParameter(FormatParamName("blogid"), Blog.CurrentInstance.Id.ToString()));
                    cmd.Parameters.Add(conn.CreateParameter(FormatParamName("id"), post.Id.ToString()));
                    cmd.Parameters.Add(conn.CreateParameter(FormatParamName("number"), verse.Number));
                    cmd.Parameters.Add(conn.CreateParameter(FormatParamName("datecreated"), DateTime.UtcNow));
                    cmd.ExecuteNonQuery();
                }
            }
        }
