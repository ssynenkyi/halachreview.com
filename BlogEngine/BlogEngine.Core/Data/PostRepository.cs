...
namespace BlogEngine.Core.Data
{
    ...
    public class PostRepository : IPostRepository
    {
        ...
        static void UpatePostVerses(Post post, string verses)
        {
            List<int> versesNumbers = new List<int>();
            if (!string.IsNullOrEmpty(verses))
            {
                var splitedVerses = verses.Split(',');
                if (splitedVerses.Any())
                {
                    foreach (var splitedVerse in splitedVerses)
                    {
                        int verseNumber;
                        if (splitedVerse.Contains("-"))
                        {
                            var diapason = splitedVerse.Split('-');
                            if (diapason.Length == 2)
                            {
                                int fromNumber;
                                int toNumber;
                                int.TryParse(diapason[0], out fromNumber);
                                int.TryParse(diapason[1], out toNumber);
                                if (fromNumber > 0 && toNumber > 0)
                                    for (int i = fromNumber; i <= toNumber; i++)
                                        if (!versesNumbers.Contains(i))
                                            versesNumbers.Add(i);
                            }
                        }
                        else
                        {
                            if (int.TryParse(splitedVerse.Trim(), out verseNumber) && !versesNumbers.Contains(verseNumber))
                                versesNumbers.Add(verseNumber);
                        }
                    }
                }
            }
            if (versesNumbers.Any())
            {
                post.VersesList = new StateList<PostVerse>();
                foreach (var versesNumber in versesNumbers)
